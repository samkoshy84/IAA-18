...
proc reg data=ameshousing3;
   model SalePrice = Basement_Area BA2 BA3 / vif;
run;
quit;
...
/*   YOU'LL LEARN PROC SQL LATER! IT'S USEFUL!      */
...
run;
quit;
/*Can check the new correlations, just for fun.*/
proc corr data=ameshousing3_center nosimple plots=matrix;;
	var  Center_BA Center_BA2 Center_BA3;
run;
...
/* Adjusting for Heterscedasticity: Robust Standard Errors */
proc model data=bootcamp.ameshousing3;
	parms B0 B1 B2 B3 B4 B5 B6 B7;
	SalePrice = B0 + B1*Gr_Liv_Area + B2*Basement_Area + B3*Garage_Area 
			 + B4*Deck_Porch_Area + B5*Lot_Area + B6*Age_Sold 
...
/* WE ARE NOT LEARNING PROC AUTOREG RIGHT NOW! THIS IS JUST FORESHADOWING! */
